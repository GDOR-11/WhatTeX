...
const greet: Behaviour = (client: wppconnect.Whatsapp) => {
    let inPeculiarSequence: false | number = false;
    client.onMessage(async message => {
        ...
        if(index === 14) {
            inPeculiarSequence = 0;
        }
        if(inPeculiarSequence !== false) {
            if(message.body.toLowerCase().includes(peculiarSequence[inPeculiarSequence])) {
